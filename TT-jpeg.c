...
TT_RGB_image *
TT_RGB_image_from_filename_jpg ( char *filename, char **comment,
	double *focal_length_35mm_equiv )
{
    ...
    return ( TT_RGB_image_from_file_jpg ( file, comment,
...
TT_RGB_image *
TT_RGB_image_from_file_jpg ( FILE *input, char **comment,
	double *focal_length_35mm_equiv )
{
    struct	    jpeg_decompress_struct cinfo;
    struct	    jpeg_DEVA_error_mgr jerr;
    jpeg_saved_marker_ptr    marker_list;
    int		    image_data_offset;	/* in units of TT_RGB! */
	...
	*focal_length_35mm_equiv = 0.0;
	...
	if ( comment != NULL ) {
		...
		if ( comment_malloc == NULL ) {
		    fprintf ( stderr,
			    "TT_RGB_image_from_file_jpg: malloc failed!\n " );
		    exit ( EXIT_FAILURE );
		}
	...
	fprintf ( stderr,
		"TT_RGB_image_from_file_jpg: input image not RGB!\n" );
	exit ( EXIT_FAILURE );
    }
    ...
    image = TT_RGB_image_new ( n_rows, n_cols );
	...
	/*
	 * Assume TT_RGB_image_new allocates image data contiguously
	 * with no padding.
	 */
	assert ( sizeof ( TT_RGB ) == 3 );	/* just checking... */
	row_pointer[0] =
	    (JSAMPROW) ( &TT_image_data ( image, 0, 0 ) + image_data_offset );
	...
	fprintf ( stderr,
		"TT_RGB_image_from_file_jpg: %d decompression warnings!\n",
...
void
TT_RGB_image_to_filename_jpg ( char *filename, TT_RGB_image *image,
	char *comment )
{
    FILE    *file;
    ...
    TT_RGB_image_to_file_jpg ( file, image, comment );

    fclose ( file );
}

void
TT_RGB_image_to_file_jpg ( FILE *output, TT_RGB_image *image, char *comment )
{
    ...
    unsigned int    n_rows, n_cols;
    int		    quality;
#include "sRGB_ICC.c"		/* hardwared binary profile */
    ...
    write_icc_profile ( &cinfo, (const JOCTET *) icc_sRGB,
	    sizeof ( icc_sRGB ) );
